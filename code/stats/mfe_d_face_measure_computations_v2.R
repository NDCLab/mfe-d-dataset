...
### and then just bin them and don't consider if they still have enough in each seperate bin.
...
proc_fileName <- "processed_data_mfe_d_Proj_v2.csv" # output filename
    ...
    if (mean(as.numeric(flanker_df$current_trial_accuracy)) >= 0.6){
      ...
      if (nrow(legit_error_incong_flankerDat) >= 8 ){
        ...
        if (num_incong_error_faces_in_surp >= 8){
          ...
          ############################################## EARLY BIN #####################################################
          ##############################################################################################################
          early_bin_error_incong_flankerDat <- filter(error_incong_flankerDat, current_trial_face_delay_time <= 0.1665)
          early_bin_correct_incong_flankerDat <- filter(incong_corrDat, current_trial_face_delay_time <= 0.1665)
          ...
          ############################################## LATE BIN #####################################################
          ##############################################################################################################
          late_bin_error_incong_flankerDat <- filter(error_incong_flankerDat, current_trial_face_delay_time > 0.1665)
          late_bin_correct_incong_flankerDat <- filter(incong_corrDat, current_trial_face_delay_time > 0.1665)
