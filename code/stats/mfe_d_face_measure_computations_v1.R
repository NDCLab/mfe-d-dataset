...
proc_fileName <- "processed_data_mfe_d_Proj_v1.csv" # output filename
...
main_df <- setNames(data.frame(matrix(ncol = 43, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "correct_hitRate", "num_incong_correctFaces_reported_new", "num_incong_correctFaces_reported_old",
                                                               "num_incong_errorFaces_reported_new", "num_incong_errorFaces_reported_old", "hit_num", "false_alrams_num",
                                                               "miss_num", "corr_rej_num", "incong_error_hit_num",
                                                               "incong_correct_hit_num", "incong_error_miss_num",
                                                               "incong_correct_miss_num", "post_error_hitRate", "post_correct_hitRate", "hitRate_error_minus_correct", "hitRate_post_error_minus_correct", "post_error_acc", "post_error_rt", "overall_dPrime"))
    ...
    if (mean(as.numeric(flanker_df$current_trial_accuracy)) >= 0.6){
      ...
      if (nrow(legit_error_incong_flankerDat) >= 8 ){
        ...
        if (num_incong_error_faces_in_surp >= 8){
          ...
          num_incong_errorFaces_reported_new <- 0
          ...
          # Post incong Error ACC and RT
          post_error_acc <- mean(as.numeric(legit_error_incong_flankerDat$post_trial_accuracy), na.rm = TRUE)
          post_error_rt <- mean(as.numeric(legit_error_incong_flankerDat$post_trial_rt), na.rm = TRUE)
          ...
          num_incong_correctFaces_reported_new <- 0
          ...
          #### post trials (under construction)
          ### number of post error faces identified as old
          num_post_errorFaces_reported_old <- 0 # this is the number of incongruent correct faces that they report as Old and will be updated in the loop below:
          num_post_errorFaces_reported_new <- 0
          for (iii in 1:nrow(error_incong_flankerDat)){
            if (!is.na(error_incong_flankerDat$post_trial_legitResponse[iii])){ # In case current trial is the last trial in the block, post trial will be NA! So, this condition will prevent from errors!
              if (error_incong_flankerDat$post_trial_legitResponse[iii] == 1){ # if this trial is legit
                if (error_incong_flankerDat$post_trial_accuracy[iii] == 1){ # post trials must be correct while they can be either incongruent or congruent.
                  temp_face_from_flanker <- error_incong_flankerDat$post_trial_face[iii]
                  temp_face_row_in_surp <- filter(surprise_df, face == temp_face_from_flanker)
                  if (nrow(temp_face_row_in_surp) == 1){ # if old incong correct face exist in the surprise_df
                    if (temp_face_row_in_surp$surpAcc == 1){ # the old face is chosen as old correctly
                      num_post_errorFaces_reported_old <- num_post_errorFaces_reported_old + 1
                    } else if (temp_face_row_in_surp$surpAcc == 0){ # the new face is chosen as old incorrectly
                      num_post_errorFaces_reported_new <- num_post_errorFaces_reported_new + 1
                    }
                  }
                }
              }
            ...
            ### number of post correct faces identified as old
            num_post_correctFaces_reported_old <- 0 # this is the number of incongruent correct faces that they report as Old and will be updated in the loop below:
            num_post_correctFaces_reported_new <- 0
            for (iii in 1:nrow(incong_corrDat)){
              if (!is.na(incong_corrDat$post_trial_legitResponse[iii])){ # In case current trial is the last trial in the block, post trial will be NA! So, this condition will prevent from errors!
                if (incong_corrDat$post_trial_legitResponse[iii] == 1){ # if this trial is legit
                  if (incong_corrDat$post_trial_accuracy[iii] == 1){ # post trials must be correct while they can be either incongruent or congruent.
                    temp_face_from_flanker <- incong_corrDat$post_trial_face[iii]
                    temp_face_row_in_surp <- filter(surprise_df, face == temp_face_from_flanker)
                    if (nrow(temp_face_row_in_surp) == 1){ # if old incong correct face exist in the surprise_df
                      if (temp_face_row_in_surp$surpAcc == 1){ # the old face is chosen as old correctly
                        num_post_correctFaces_reported_old <- num_post_correctFaces_reported_old + 1
                      } else if (temp_face_row_in_surp$surpAcc == 0){ # the new face is chosen as old incorrectly
                        num_post_correctFaces_reported_new <- num_post_correctFaces_reported_new + 1
                      }
                    }
                  }
                }
          ...
          # post hit rate values will be NAs.
          if (num_post_errorFaces_reported_new + num_post_errorFaces_reported_old >= 8){
            ...
            hitRate_post_error_minus_correct <- post_error_hitRate - post_correct_hitRate

          } else if (num_post_errorFaces_reported_new + num_post_errorFaces_reported_old < 8){
            post_error_hitRate <- NA
            post_correct_hitRate <- NA
            hitRate_post_error_minus_correct <- NA
          }
...
redcapDat <- read.csv(file = "/Users/kihossei/Library/CloudStorage/GoogleDrive-hosseinikianoosh@gmail.com/My Drive/My Digital Life/Professional/GitHub_Repos/mfe-d-dataset/derivatives/redcap/202408v0memoryforerr_SCRD_2024-10-01_2352.csv")
