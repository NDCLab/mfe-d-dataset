...
cover_background_2 = visual.ImageStim(
    win=win,
    name='cover_background_2', units='deg', 
    image='sin', mask=None, anchor='center',
...
# The delay is between 0 and 333 ms.
face_delay = [i / 1000 for i in list(range(0, 334))]
...
for thisPrac_block_loop in prac_block_loop:
    ...
    for thisPrac_trial_loop in prac_trial_loop:
        ...
        # keep track of which components have finished
        prac_stimRoutineComponents = [cover_background_2, prac_centerImg, prac_rightImg1, prac_rightImg2, prac_leftImg1, prac_leftImg2, prac_fixImg, prac_stim_keyResp]
        ...
        prac_trial_loop.addData('current_flanker_routine_time', current_flanker_routine_time)
        ...
        # We will compute the display duration for the face image based on 
        # when participant responded in the prior routine. 
        # 
        face_delay_time = np.random.choice(face_delay)
        faceDisp_plus_faceDelay_time = thisISI - current_flanker_routine_time 
        
        
        # save this ISI to our output file
        prac_trial_loop.addData('face_delay_time', face_delay_time)
        prac_trial_loop.addData('faceDisp_plus_faceDelay_time', faceDisp_plus_faceDelay_time)
        ...
        while continueRoutine:
            ...
            if prac_delayed_face.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prac_delayed_face.tStartRefresh + faceDisp_plus_faceDelay_time-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if prac_fixImg_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prac_fixImg_2.tStartRefresh + faceDisp_plus_faceDelay_time-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if prac_stim_keyResp2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prac_stim_keyResp2.tStartRefresh + faceDisp_plus_faceDelay_time-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    for thisTask_trial_loop in task_trial_loop:
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            if task1_stim_keyResp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > task1_stim_keyResp.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        task_trial_loop.addData('faceDisp_plus_faceDelay_time', faceDisp_plus_faceDelay_time)
        ...
        while continueRoutine:
            ...
            if task_delayed_face.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > task_delayed_face.tStartRefresh + faceDisp_plus_faceDelay_time-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if task_fixImg_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > task_fixImg_2.tStartRefresh + faceDisp_plus_faceDelay_time-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if task_stim_keyResp2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > task_stim_keyResp2.tStartRefresh + faceDisp_plus_faceDelay_time-frameTolerance:
                    # keep track of stop time/frame for later
